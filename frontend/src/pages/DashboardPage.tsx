  ...
  const handleCreateSpaceSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        alert(res?.message || "Failed to create space. Please try again.");
      }
    } catch (error) {
  ...
  const handleDeleteSpaceSubmit = async () => {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        alert(res?.message || "Failed to delete space.");
      }
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10 pb-6 border-b border-white/10">
          ...
          <button
            ...
            className="self-start md:self-center px-5 py-3 rounded-xl bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 text-white text-sm font-semibold shadow-lg shadow-pink-500/25 hover:scale-[1.02] active:scale-95 transition-all cursor-pointer flex items-center gap-2"
          >
              ...
              {spaces.map((space) => (
                ...
                >
                  ...
                  <div className="mt-8 flex items-center justify-between gap-3 pt-4 border-t border-white/[0.05]">
                    ...
                    <button
                      onClick={() => console.log("Navigate to /spaces/" + space.id)}
          ...
          <div className="relative w-full max-w-md rounded-2xl bg-[#09061a] border border-white/[0.1] shadow-2xl p-6 overflow-hidden">
            ...
            <form onSubmit={handleCreateSpaceSubmit} className="space-y-4">
              ...
              <div className="pt-4 flex items-center justify-end gap-3 border-t border-white/[0.06]">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 rounded-lg bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 text-white text-xs font-semibold shadow-lg shadow-pink-500/20 active:scale-95 transition-all cursor-pointer"
                >
                  Create Now
                </button>
              </div>
